...
def read_grid_id(self):
    for self.layer in list(QgsProject.instance().mapLayers().values()):
        if self.layer.name() == ("mf_obs (APEX-MODFLOW)"):
            self.dlg.groupBox_plot_wt.setEnabled(True)
            self.layer = QgsProject.instance().mapLayersByName("mf_obs (APEX-MODFLOW)")[0]
            ...
            # items to string
            itostr = map(str, sorted_grid_id)
            # a = sorted(a, key=lambda x: float(x))
            self.dlg.comboBox_grid_id.clear()
            # self.dlg.comboBox_sub_number.addItem('')
            self.dlg.comboBox_grid_id.addItems(itostr) # in addItem list should contain string numbers
        else:
    ...
    if self.dlg.checkBox_wt_obd.isChecked():
        ...
        try:
            ...
            if self.dlg.checkBox_depthTowater.isChecked():
                ...
                if (len(df3[wt_ob]) > 1):
                    ...
                    with open(
                            os.path.join(
                                outfolder, "apexmf_grid_id(" + str(grid_id) + ")"+
                                "_ob("+ str(wt_ob)+")_d_dtw.txt"), 'w') as f:
                        f.write("# apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                                +")_d_dtw.txt is created by APEXMOD plugin "+ version + time + "\n")
                    ...
                    msgBox.setText(
                                "'apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                ...
                if (len(df3[wt_ob]) > 1):
                    ...
                    # ------------ Export Data to file -------------- #
                    with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")"+
                        "_ob("+ str(wt_ob)+")_d_wt.txt"), 'w') as f:
                        f.write("# apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                                +")_d_wt.txt is created by APEXMOD plugin "+ version + time + "\n")
                        df3.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                        f.write('\n')
                        f.write("# Statistics\n")
                        f.write("Nash–Sutcliffe: " + str('{:.4f}'.format(dNS) + "\n"))
                    ...
                    msgBox.setWindowIcon(QtGui.QIcon(':/APEXMOD/pics/am_icon.png'))
                    msgBox.setWindowTitle("Exported!")
                    msgBox.setText("'apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
        ...
        try:
            if self.dlg.checkBox_depthTowater.isChecked():
                ...
                # ------------ Export Data to file -------------- #
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")_d_dtw.txt"), 'w') as f:
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")_d_dtw.txt is created by APEXMOD plugin "
                        + version + time + "\n")
                    df.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
                ...
                msgBox.setWindowIcon(QtGui.QIcon(':/APEXMOD/pics/am_icon.png'))
                msgBox.setWindowTitle("Exported!")
                msgBox.setText("'apexmf_grid_id(" + str(grid_id) 
                    + ")_d_dtw.txt' file is exported to your 'exported_files' folder!")
                msgBox.exec_()
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")_d_wt.txt"), 'w') as f:
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")_d_wt.txt is created by APEXMOD plugin "
                        + version + time + "\n")
                    df.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
    ...
    if self.dlg.checkBox_wt_obd.isChecked():
        ...
        try:
            ...
            if self.dlg.checkBox_depthTowater.isChecked():
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")"+
                    ...
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                            +")_m_dtw.txt is created by APEXMOD plugin "+ version + time + "\n")
                    df3.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
                    f.write("Nash–Sutcliffe: " + str('{:.4f}'.format(dNS) + "\n"))
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")"+
                    ...
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                            +")_m_wt.txt is created by APEXMOD plugin "+ version + time + "\n")
                    df3.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
                    f.write("Nash–Sutcliffe: " + str('{:.4f}'.format(dNS) + "\n"))
        ...
        try:
            if self.dlg.checkBox_depthTowater.isChecked():
                ...
                dfm = df.resample('M').mean()
                # ------------ Export Data to file -------------- #
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")_m_dtw.txt"), 'w') as f:
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")_m_dtw.txt is created by APEXMOD plugin "
                        + version + time + "\n")
                    dfm.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
                    f.write("Nash–Sutcliffe: ---\n")
                    f.write("R-squared: ---\n")
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")_m_wt.txt"), 'w') as f:
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")_m_wt.txt is created by APEXMOD plugin "
                        + version + time + "\n")
                    dfm.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
    ...
    if self.dlg.checkBox_wt_obd.isChecked():
        ...
        try:
            ...
            if self.dlg.checkBox_depthTowater.isChecked():
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")"+
                    ...
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                            +")_a_dtw.txt is created by APEXMOD plugin "+ version + time + "\n")
                    df3.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
                    f.write("Nash–Sutcliffe: " + str('{:.4f}'.format(dNS) + "\n"))
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")"+
                    ...
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")"+"_ob("+ str(wt_ob)
                            +")_a_wt.txt is created by APEXMOD plugin "+ version + time + "\n")
                    df3.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
                    f.write("Nash–Sutcliffe: " + str('{:.4f}'.format(dNS) + "\n"))
        ...
        try:
            if self.dlg.checkBox_depthTowater.isChecked():
                ...
                # ------------ Export Data to file -------------- #
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")_a_dtw.txt"), 'w') as f:
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")_a_dtw.txt is created by APEXMOD plugin "
                        + version + time + "\n")
                ...
                with open(os.path.join(outfolder, "apexmf_grid_id(" + str(grid_id) + ")_a_wt.txt"), 'w') as f:
                    f.write("# apexmf_grid_id(" + str(grid_id) + ")_a_wt.txt is created by APEXMOD plugin "
                        + version + time + "\n")
                    dfa.to_csv(f, index_label = "Date", sep = '\t', float_format='%10.4f', lineterminator='\n', encoding='utf-8')
                    f.write('\n')
                    f.write("# Statistics\n")
