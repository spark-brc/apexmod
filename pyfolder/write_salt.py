...
def create_icbund(self):
    ...
    if self.layer.dataProvider().fields().indexFromName("icbund") == -1:
        ...
        # Find .dis file and read number of rows, cols, x spacing, and y spacing (not allowed to change)
        for filename in glob.glob(str(APEXMOD_path_dict['MODFLOW'])+"/*.bas"):
            with open(filename, "r") as f:
                data = []
                for line in f.readlines():
                    if not line.startswith("#"):
...
def write_rt3d_inputs(self):
    ...
    with open(
        ...
        f.write("'MODFLOW/{}'           INBTN=1			Basic Transport Package\n".format(rt3d_name+'.btn'))
        f.write("'MODFLOW/{}'			INADV=2			Advection Package\n".format(rt3d_name+'.adv'))
        f.write("'MODFLOW/{}'			INDSP=3			Dispersion Package\n".format(rt3d_name+'.dsp'))
        f.write("'MODFLOW/{}'			INSSM=4			Source/Sink Mixing Package\n".format(rt3d_name+'.ssm'))
        f.write("'MODFLOW/{}'			INRCT=5			Reaction Package\n".format(rt3d_name+'.rct'))
        f.write("'MODFLOW/{}'			INGCG=6			Implicit Solver Package\n".format(rt3d_name+'.gcg'))
        f.write("'MODFLOW/{}'		    OUTRES=10		Restart File\n".format(rt3d_name+'.restart'))
