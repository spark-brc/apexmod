...
# from APEXMOD.APEXMOD import dirs_and_paths --> this is not working why?
...
def MF_grid(self):  # Create fishnet based on MODFLOW dis file
    ...
    layer = QgsProject.instance().mapLayersByName("sub (APEX)")[0]
    crs = layer.crs()
...
def mf_riv1(self):
    ...
    for filename in glob.glob(str(APEXMOD_path_dict['MODFLOW'])+"/*.riv"):
        with open(filename, "r") as f:
            data = []
            for line in f.readlines():
                if not line.startswith("#"):
...
def mf_riv2(self):
    APEXMOD_path_dict = self.dirs_and_paths()
    input1 = QgsProject.instance().mapLayersByName("mf_grid (MODFLOW)")[0]
    input2 = QgsProject.instance().mapLayersByName("riv (APEX)")[0]
...
#   input2 = QgsProject.instance().mapLayersByName("DEM (APEX)")[0]
...
def mf_riv3(self):
    ...
    output_dir1 = APEXMOD_path_dict['org_shps']
    ...
    output_dir2 = APEXMOD_path_dict['org_shps']
...
def overwriteRivPac(self):
    ...
    riv_files = [f for f in glob.glob(str(APEXMOD_path_dict['MODFLOW'])+"/*.riv")]
    if len(riv_files) == 1:
        # ------------ Export Data to file -------------- #
        version = "version 1.5."
        time = datetime.now().strftime('- %m/%d/%y %H:%M:%S -')
        with open(riv_files[0], "w", newline='') as f:
            ...
            first_row = ["# "+ os.path.basename(riv_files[0]) +" file is overwritten by APEXMOD plugin " + version + time]
...
def rivInfoTo_mf_riv2(self): 
    ...
    APEXMOD_path_dict = self.dirs_and_paths()
    # try:
    river_grid = QgsProject.instance().mapLayersByName("river_grid (APEX-MODFLOW)")[0]
...
def use_obs_points(self):
    ...
    if inFileName:
        ...
        Out_folder = APEXMOD_path_dict['org_shps']
...
def select_obs_grids(self):
    ...
    # Deselect the features
    input1.removeSelection()
    layer = QgsVectorLayer(mf_obs_shapefile, '{0} ({1})'.format("mf_obs","APEX-MODFLOW"), 'ogr')

    # Put in the group
    root = QgsProject.instance().layerTreeRoot()
    swat_group = root.findGroup("APEX-MODFLOW") 
    QgsProject.instance().addMapLayer(layer, False)
    swat_group.insertChildNode(0, QgsLayerTreeLayer(layer))
    layer = QgsProject.instance().addMapLayer(layer)
...
def mf_obs_shapefile(self):
    ...
    if inFileName:
        settings.setValue('/APEXMOD/LastInputPath', os.path.dirname(str(inFileName)))
        Out_folder = APEXMOD_path_dict['apexmf_shps']
        inInfo = QFileInfo(inFileName)
        inFile = inInfo.fileName()
        pattern = os.path.splitext(inFileName)[0] + '.*'
        
        # inName = os.path.splitext(inFile)[0]
        inName = 'mf_obs_apexmf'
        for f in glob.iglob(pattern):
            suffix = os.path.splitext(f)[1]
            if os.name == 'nt':
                outfile = ntpath.join(Out_folder, inName + suffix)
        ...
        layer = QgsVectorLayer(mf_obs_shp, '{0} ({1})'.format("mf_obs","APEX-MODFLOW"), 'ogr')
...
def export_modflow_obs(self):
    ...
    with open(output_file, "w", newline='') as f:
        writer = csv.writer(f, delimiter = '\t')
        first_row = ["# modflow.obs file is created by APEXMOD plugin " + version + time]
...
def create_modflow_mfn(self):
    ...
    for filename in glob.glob(wd + "/*.nam"):
        ...
        info = "# modflow.mfn file, generated by APEXMOD plugin "+version+ time +'\n'
        lines = []
        lines.append(info)
        ...
        for line in data:
            if line.startswith("#"):
                ...
                line = line.split()[0] + "\t" + str(b) + "\t" + "MODFLOW\\"+ line.split()[2]
                lines.append(line + '\n')
...
def modify_modflow_oc(self):
    ...
    for filename in glob.glob(wd + "/*.oc"):
        version = "version 1.5."
        time = datetime.datetime.now().strftime(' - %m/%d/%y %H:%M:%S -')
        info = "# Output Control file, modified by APEXMOD plugin "+version+ time +'\n'
            ...
            for line in infile:
                ...
                try:
                    ...
                    if a == 'ss' or a == 'SS':
                        ...
                        msgBox.exec_()
                        file = "editLog.txt"
                        with open(os.path.join(APEXMOD_path_dict['MODFLOW'], file), 'a') as outfile:
...
###### NOTE: this might be an option for MODFLOW-USG, but for now not used for efficiency
