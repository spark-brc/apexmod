...
def create_irrig_mf(self):
    ...
    # export file
    APEXMOD_path_dict = self.dirs_and_paths()
    name = "apexmf_irrigate.txt"
    out_dir = APEXMOD_path_dict['MODFLOW']
    ...
    L1 = [
        ...
        ", created by APEXMOD plugin " + version + time]
    ...
    msgBox.setWindowTitle("Created!")
    msgBox.setText(
                    "'apexmf_irrigate.txt' file has been created successfully!"
                    )
    msgBox.exec_()
...
def link_drain(self):
    ...
    if not root.findGroup("Scenarios"):
        sm_tree = root.findGroup("APEX-MODFLOW")
    ...
    for filename in glob.glob(str(APEXMOD_path_dict['MODFLOW'])+"/*.drn"):
        with open(filename, "r") as f:
            data = []
            for line in f.readlines():
                if not line.startswith("#"):
    ...
    # Create layer field
    # lyrProvider = input2.dataProvider()
    ...
    #     # add layer number
    #     for f in feats:
    #         input2.changeAttributeValue(f.id(), lyrIdx, 1)
    #     input2.commitChanges()
    ...
    msgBox.setWindowTitle("Created!")
    msgBox.setText(
                    "'drain2sub.shp' file has been created!"
                    )
    msgBox.exec_()
...
def create_drain2sub(self):
    ...
    msgBox.setWindowTitle("Created!")
    msgBox.setText(
                    "'apexmf_drain2sub.txt' file has been created successfully!"
                    )
    msgBox.exec_()
...
def use_irrig_swat_pts(self):
    ...
    if inFileName:
        ...
        Out_folder = APEXMOD_path_dict['Scenarios']
...
def write_irrig_swat(self):
    ...
    nhru = len(data_f)
    L1 = [
        "# Irrigation Pumping File for APEX-MODFLOW" +
        ", created by APEXMOD plugin " + version + time]
...
def modify_wel(self):
    ...
    if any(inFile.endswith('.wel') for inFile in os.listdir(wd)):
        info = "# Well Package input file, modified by APEXMOD plugin "+version + time + '\n' 
        ...
        msgBox.exec_()
        lines = []
        info = "# Well Package input file, created by APEXMOD plugin " + version + time + '\n'
        lines.append(info)
